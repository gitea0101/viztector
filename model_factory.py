...
class Network(nn.Module):
    ...
    #     else:
    #       self.model.load_state_dict(torch.load(pretrained_model, map_location=torch.device('cpu')), strict=False)

    def load_pretrained_model(self, pretrained_model):
        print("Loading from {}".format(pretrained_model))
        
        if torch.cuda.is_available():
            pre_model_dict = torch.load(pretrained_model)
        else:
            pre_model_dict = torch.load(pretrained_model, map_location=torch.device('cpu'))
                                        
        for key, val in self.model.named_parameters():
            if key in pre_model_dict.keys():
    ...
    def load_model(self, iteration):
        ...
        if torch.cuda.is_available():
          ...
          self.model.load_state_dict(torch.load(cache_file, map_location=torch.device('cpu')))
